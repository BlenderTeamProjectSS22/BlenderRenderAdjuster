...
class Camera:
    ...
    def set_camera_rotation(self, x_rotation: float, y_rotation: float, z_rotation: float):
    ...
    def drive_by(self, frames: int, startPoint: list, endPoint: list, Rotation: list, track: bool):
    ...
    def set_mode(self, mode: str, track: bool, object: bpy.types.Object):
        if mode == "track":
            track == True
            self.cam.constraints.new("TRACK_TO")
            self.cam.constraints["Track To"].target = object
        else:
            track == False
            bpy.data.objects[self.name].constraints["Track To"].delete()
